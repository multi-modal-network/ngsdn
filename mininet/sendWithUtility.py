...
def generate_ndn_pkt(ethertype, srcVmx, srcId, dstVmx, dstId):
    ...
    pkt = pkt / Raw(load=struct.pack("!LLLLLLLLL", 0x6fd0020, 0x80c0804, name_component_src,
                                     ...
                                     , (((name_component_dst & 0xffff)) << 16) | 0x1e00, 0x18020000, 0x19020000,
                                     0x1b020000, 0x1a020000 | content))
    pkt.show2()
    return pkt
...
def generate_ip_pkt(ethertype, srcVmx, srcId, dstVmx, dstId):
    ...
    pkt = Ether(type=ethertype, src=getMacByVmx(srcVmx), dst=getMacByVmx(dstVmx)) / IP(src=srcIP, dst=dstIP) / TCP(
        dport=1234, sport=49152)
    pkt.show2()
    return pkt
...
def customFlexIP(vmx, i):
    ...
    if 144 <= i < 152:
        ...
        return "F6F0{:04X}F1{:08X}".format(F0,
                                           F1), length_index + length_index + length_extendable_F0 + length_index + length_extendable_F1, format_hierarchical
    elif 184 <= i < 192:
        return "F6F0{:04X}F2{:016X}".format(F0,
                                            F2), length_index + length_index + length_extendable_F0 + length_index + length_extendable_F2, format_hierarchical
    elif 192 <= i < 200:
        return "F6F0{:04X}F4{:064X}".format(F0,
                                            F4), length_index + length_index + length_extendable_F0 + length_index + length_extendable_F4, format_hierarchical
    elif 200 <= i < 208:
        return "F6F1{:08X}F2{:016X}".format(F1,
                                            F2), length_index + length_index + length_extendable_F1 + length_index + length_extendable_F2, format_hierarchical
    elif 208 <= i < 216:
        return "F6F1{:08X}F4{:064X}".format(F1,
                                            F4), length_index + length_index + length_extendable_F1 + length_index + length_extendable_F4, format_hierarchical
    elif 216 <= i < 224:
        return "F6F2{:016X}F4{:064X}".format(F2,
                                             F4), length_index + length_index + length_extendable_F2 + length_index + length_extendable_F4, format_hierarchical
    elif 224 <= i < 232:
        return "F7F0{:04X}F1{:08X}F2{:016X}".format(F0, F1,
                                                    F2), length_index + length_index + length_extendable_F0 + length_index + length_extendable_F1 + length_index + length_extendable_F2, format_hierarchical
    elif 232 <= i < 240:
        return "F7F0{:04X}F1{:08X}F4{:064X}".format(F0, F1,
                                                    F4), length_index + length_index + length_extendable_F0 + length_index + length_extendable_F1 + length_index + length_extendable_F4, format_hierarchical
    elif 240 <= i < 248:
        return "F7F0{:04X}F2{:016X}F4{:064X}".format(F0, F2,
                                                     F4), length_index + length_index + length_extendable_F0 + length_index + length_extendable_F2 + length_index + length_extendable_F4, format_hierarchical
    elif 248 <= i < 256:
        return "F7F1{:08X}F2{:016X}F4{:064X}".format(F1, F2,
                                                     F4), length_index + length_index + length_extendable_F1 + length_index + length_extendable_F2 + length_index + length_extendable_F4, format_hierarchical
...
def sendInfoToUtility(src_host, dst_host, modal_type):
    ...
    json_data = json.dumps(data, 4)
    url = "https://218.199.84.172:8181/api/traffic"
...
def main():
    ...
    print("modal_type:%s, frequency:%d, source_host:%s, destination_host:%s" % (
    modal_type, frequency, source_host, destination_host))
    
    sendInfoToUtility(source_host, destination_host, modal_type)
    ...
    for i in range(1, frequency + 1):
        ...
        if cnt_after == cnt_before + 1:
            ...
            if line_after[0] == modal_type and int(line_after[1]) == source_host and int(
                    line_after[2]) == destination_host:
                print("resend!")
                sendp(pkt, iface=iface, verbose=False)
